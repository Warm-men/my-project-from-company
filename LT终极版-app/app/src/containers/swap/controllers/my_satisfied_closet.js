...
@observer
class MySatisfiedClosetContainer extends AuthenticationComponent {
  constructor(props) {
    super(props)
    ...
    this.initFilters = {
      per_page: 20,
      sort: 'perfect_stock_first',
      filter_terms:
        params && params.type && params.type === 'all' ? [] : [params.type],
      color_families: [],
      temperature: []
    }
    this.state = {
      ...
      showFilter: false,
      selectedType: params && params.type ? params.type : 'all',
  ...
  _addObservers = () => {
    this.scrollY.addListener(({ value }) => {
      if (value > p2d(145) && !this.state.showFilter) {
    ...
    this.listeners.push(
      DeviceEventEmitter.addListener(
        'onRefreshSatisfiedProductsCloset',
        ({ filters }) => {
          this.setState(
            {
              filters: {
  ...
  _openFilterView = () => {
    ...
    const productType =
      selectedType === 'all' ? 'satisfied' : 'satisfied-' + selectedType
    const { filter_terms } = this.state.filters
    ...
    DeviceEventEmitter.emit('currentSatisfiedFilterTerms', {
  ...
  _getClosetListData = () => {
    ...
    this.idCounter = QNetwork(
      SERVICE_TYPES.closet.QUERY_SATISFIED_CLOSET,
      variables,
      response => {
  ...
  _getReportData = index => {
    ...
    const column =
      routeName === 'SatisfiedCloset'
        ? Column.SatisfiedCloset
        : Column.SwapSatisfiedCloset
  ...
  _onPressToggleSwitch = isOn => {
    const sort = isOn ? 'perfect_stock_first' : 'perfect_closet_created_first'
    this.setState(
      {
  ...
  _productListHeader = () => {
    ...
    return (
      <FilterHeader
        inToggleSwitchOn={this.state.inToggleSwitchOn}
...
export default MySatisfiedClosetContainer
