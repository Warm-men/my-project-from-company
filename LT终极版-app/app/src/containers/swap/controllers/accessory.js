...
@observer
export default class SwapAccessoryContainer extends Component {
  constructor(props) {
    super(props)
    this.state = {
      page: 1,
  ...
  _addObservers = () => {
    this.scrollY.addListener(({ value }) => {
      if (value > 135 && !this.state.showFilter) {
    ...
    this.listeners.push(
      DeviceEventEmitter.addListener(
        'onRefreshSwapColletionAccessory',
        data => {
  ...
  _getReportData = index => {
    ...
    const column = Column.ToteSwapAccessory
